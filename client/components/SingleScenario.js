...
import emotionClassifier from '../../script/models/emotionclassifier'
import emotionModel from '../../script/models/emotionmodel.js'
...
class Learn extends React.PureComponent {
  ...
  async componentDidMount() {
    ...
    const {data} = await axios.get(
      `/api/scenarios/${this.props.match.params.scenarioId}`
    )
    this.setState(
      {
        scenario: data,
  ...
  drawLoop = () => {
    ...
    this.state.overlayCC.clearRect(
      0,
      0,
      this.state.vidWidth,
    ...
    if (er) {
      for (var i = 0; i < er.length; i++) {
        if (er[i].value > 0.4) {
          ...
          if (
            this.state.currentEmotion === this.state.scenario.correctEmotion
          ) {
            this.setState({
          ...
          document.getElementById(`icon-${er[i].emotion}`).style.visibility =
            'visible'
        } else {
          document.getElementById(`icon-${er[i].emotion}`).style.visibility =
            'hidden'
        ...
        <div className="row">
          <div className="col">
            ...
            <div id="emotion_container">
              ...
              {this.state.scenario ? (
                <div>
                  <h4>{this.state.scenario.prompt}</h4>
                  <div>
                    {this.state.currentEmotion !== '' &&
                    this.state.scenario.correctEmotion !==
                      this.state.successfulEmotion ? (
                      <div className="alert alert-primary" role="alert">
                        <h4>
                          You are currently showing me that you are{' '}
                          <u>{this.state.currentEmotion}</u>
                        </h4>
                      </div>
                    ) : (
                      ''
                  ...
                  >
                    ...
                    this.state.currentEmotion !== '' ? (
                      <div>
                        <p>
                          What facial expression should you make in this case?
                        </p>
                        <h4>Hit 'Start' when you're ready!</h4>
                      </div>
                    ) : (
                      ''
                    )}
            ...
            >
              ...
              this.state.successfulEmotion ? (
                <div>
                  ...
                  <audio src={this.state.scenario.audioSuccessURL} autoPlay />
                  <h4>Correct!</h4>
                  <div className="alert alert-primary" role="alert">
                    <h4>
                      You should be <u>{this.state.successfulEmotion}</u> in
                      this case!
                    </h4>
                  </div>
                </div>
